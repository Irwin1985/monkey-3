...
func TestBangOperatoor(t *testing.T) {
	tests := []struct {
		input    string
		expected bool
	}{
...
func TestErrorHandling(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if errObj.Message != tt.expectedMessage {
			t.Errorf("wrong error message for input %q. expected=%q, got=%q", tt.input, tt.expectedMessage, errObj.Message)
		}
	}
}
...
func TestStringLConcatenation(t *testing.T) {
	input := `"Hello" + " " + "World!"`
	evaluated := testEval(input)
	str, ok := evaluated.(*object.String)
	if !ok {
...
func ExampleEval_puts_integer() {
	input := `puts(1)`
	testEval(input)
	// Output: 1
}

func ExampleEval_puts_string() {
	input := `puts("hello monkey")`
	testEval(input)
	// Output: hello monkey
}

func ExampleEval_puts_multipleValues() {
